# BookPrintAPI Node.js SDK — Examples

이 폴더의 예제는 **모두 백엔드(서버) Node 프로세스에서 실행되는 것을 전제**합니다.
...
# 2. .env 작성
#   SWEETBOOK_API_KEY=sk_live_xxx   또는   sk_test_xxx
#   SWEETBOOK_ENV=sandbox           # 또는 live
#   SWEETBOOK_WEBHOOK_SECRET=whsk_...  # 웹훅 사용 시
```
...
| 파일 | 역할 | 특징 |
|---|---|---|
| `01_create_book.js` | 책 생성 → 표지 → 내지 → 최종화 (짧은 데모) | 최소 흐름만 |
| `server_pipeline.js` | **책 생성 → 주문 E2E 파이프라인** | 실서비스의 백그라운드 작업 이식용 |
| `02_order.js` | 충전금 확인 → 견적 → 주문 | 단일 도메인 학습용 |
| `03_webhook_server.js` | 웹훅 수신 + 서명 검증 서버 | http 모듈 기반 |

## server_pipeline.js — 책 생성 파이프라인 시퀀스
...
```
 (큐/스케줄러)
 BullMQ, Agenda, node-cron  ──→  server_pipeline.js 로직
                                        ↓
                                   BookPrint API
```

페이지 수가 많은 책은 전체 흐름이 수 분 걸릴 수 있습니다.
**동기 HTTP 응답 안에서 실행하지 말고** 큐 태스크로 분리하세요.

## 웹훅 수신 (03_webhook_server.js)

주문/제작/배송 상태 변경은 **폴링이 아니라 웹훅**으로 수신하세요.
`verifySignature()`로 반드시 서명을 검증하고, 200 응답을 2초 이내 반환해야 재시도가 걸리지 않습니다.
