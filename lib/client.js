...
const { HelpersClient } = require('./helpers');
...
class BooksClient extends BaseClient {
  async list(params = {}) {
    ...
    return new ResponseParser(body).toListResult('books');
  }

  async create(data) {
    const { bookSpecUid, title, creationType = 'TEMPLATE', externalRef, pageCount, ...extraData } = data;
    if (!bookSpecUid) throw new SweetbookValidationError('bookSpecUid is required', 'bookSpecUid');
    if ((creationType === 'PDF_UPLOAD' || creationType === 'MIX_COVER_TEMPLATE')
        && (typeof pageCount !== 'number' || pageCount <= 0)) {
      throw new SweetbookValidationError(
        `creationType=${creationType} 는 pageCount(내지 페이지수, >0)가 필수입니다.`,
        'pageCount'
      );
    }
...
class PhotosClient extends BaseClient {
  ...
  async list(bookUid) {
    ...
    return new ResponseParser(body).toListResult('photos');
  }
...
class CoversClient extends BaseClient {
   ...
   * 예: 템플릿이 `coverPhoto` binding을 요구하면 `bindingFiles: { coverPhoto: file }` 로 전달.
   *
   * @param {string} bookUid
   * @param {string} templateUid
   * @param {object} parameters — 템플릿 파라미터 (텍스트 / 사진 fileName 참조 등)
   * @param {object|Array} optionsOrFiles — 권장: { bindingFiles: { coverPhoto: file, ... } }.
   *     하위 호환: Array<File> 도 받지만 deprecated (모든 파일을 'files' 단일 필드명으로 보내 서버 거부).
   */
  async create(bookUid, templateUid, parameters, optionsOrFiles) {
    this._requireParam(bookUid, 'bookUid');
    this._requireParam(templateUid, 'templateUid');
    const buildOptions = Array.isArray(optionsOrFiles)
      ? { files: optionsOrFiles, fileFieldName: 'files' }
      : (optionsOrFiles || {});
    const fd = this._buildTemplateFormData(templateUid, parameters, buildOptions);
...
class ContentsClient extends BaseClient {
   ...
   *
   * multipart 파일 part 이름은 **템플릿이 정의한 binding 이름**과 일치해야 합니다.
   * 예: 템플릿이 `mainPhoto`, `subPhoto` binding 을 요구하면
   * `bindingFiles: { mainPhoto: f1, subPhoto: f2 }` 로 전달.
   *
   * @param {string} bookUid
   * @param {string} templateUid
   * @param {object} parameters
   * @param {object} options — { bindingFiles?, files?(deprecated), breakBefore? }
   */
  async insert(bookUid, templateUid, parameters, options = {}) {
    this._requireParam(bookUid, 'bookUid');
    this._requireParam(templateUid, 'templateUid');
    const { files, bindingFiles, breakBefore } = options;
    const fd = this._buildTemplateFormData(templateUid, parameters, {
      bindingFiles,
      files,
      fileFieldName: 'rowPhotos',
    });
    const params = {};
...
class OrdersClient extends BaseClient {
  ...
  async list(params = {}) {
    ...
    return new ResponseParser(body).toListResult('orders');
  }
...
// ============================================================
// BookSpecs Client — 상품 스펙 목록/상세 조회 (읽기 전용)
// ============================================================

class BookSpecsClient extends BaseClient {
  async list(options = {}) {
    const params = options.accountUid ? { accountUid: options.accountUid } : undefined;
    const body = await this._get('/book-specs', params);
    // envelope 통일 (commit 6fbf346) 후 data: [...] 직접 배열. 옛 형태 호환 위해 toListResult.
    return new ResponseParser(body).toListResult('bookSpecs');
  }

  async get(bookSpecUid, options = {}) {
    this._requireParam(bookSpecUid, 'bookSpecUid');
    const params = options.accountUid ? { accountUid: options.accountUid } : undefined;
    const body = await this._get(`/book-specs/${bookSpecUid}`, params);
    return new ResponseParser(body).getDict();
  }
}
...
class TemplatesClient extends BaseClient {
  async list(params = {}) {
    ...
    return new ResponseParser(body).toListResult('templates');
  }
   ...
   *
   * AI 에이전트 / 페이로드 검증 / codegen 용도. 응답 data 는 JSON Schema 문서로,
   * properties 각 항목의 x-binding 으로 binding 종류 식별 (text / file / gallery /
   * collageGallery / rowGallery). required 명시되지 않은 키는 응답 시점에 자동
   * 필수(true) 처리.
   */
  async getSchema(templateUid) {
    this._requireParam(templateUid, 'templateUid');
    const body = await this._get(`/templates/${templateUid}/schema`);
    return new ResponseParser(body).getDict();
  }
}
...
class CreditsClient extends BaseClient {
  ...
  async transactions(params = {}) {
    ...
    // envelope 통일 후 data: [...] 배열 (pagination 없음). 옛 호환 위해 toListResult.
    return new ResponseParser(body).toListResult('transactions');
  }
...
class SweetbookClient {
  ...
  constructor(options = {}) {
    ...
    this.helpers = new HelpersClient(this);
  }
}
