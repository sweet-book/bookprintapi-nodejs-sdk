...
/**
 * 헬퍼 단계 식별자 — 11_sdk_helpers_design.md § 3.2
 */
const HelperStage = Object.freeze({
 ...
 * C03 확정 시 표준 errorCode 로 매핑.
 */
const HelperErrorCodes = Object.freeze({
 ...
 * 속성:
 * - stage: HelperStage 값
 * - code: HelperErrorCodes 값
 * - bookUid: BOOK_CREATE 성공 후부터 채워짐
 * - orderUid: uploadPdfAndOrder 의 ORDER_CREATE 성공 후만
 * - partial: 단계별 부분 성공 정보 객체
 * - cause: 원 예외 (대부분 SweetbookApiError)
 * - contentIndex: stage=CONTENT_INSERT 일 때 0-based 페이지 index
 */
class SweetbookHelperError extends Error {
  constructor(message, {
    stage,
    code,
    bookUid = null,
    ...
    super(message);
    this.name = 'SweetbookHelperError';
    this.stage = stage;
    this.code = code;
  ...
  /** 사용자 표시용 메시지. cause 가 SweetbookApiError 면 그쪽으로 위임. */
  userMessage() {
    if (this.cause instanceof SweetbookApiError) return this.cause.userMessage();
    return this.message;
  }

  toString() {
    const stageLabel = this.contentIndex !== null
      ? `[${this.stage}#${this.contentIndex}]`
      : `[${this.stage}]`;
    const parts = [stageLabel, this.message];
    if (this.code) parts.push(`(${this.code})`);
    if (this.bookUid) parts.push(`bookUid=${this.bookUid}`);
    return parts.join(' ');
  }
}
...
class ResponseParser {
  ...
  getList() {
    ...
    if (d && typeof d === 'object') {
      ...
      // 마지막 fallback: data 객체에서 첫 번째 배열 값
  ...
  getPagination() {
    ...
    if (d && typeof d === 'object' && !Array.isArray(d)) {
      ...
      // 구 photos: data.totalCount → pagination.total
      if (typeof d.totalCount === 'number') {
        return { total: d.totalCount };
      }
    }
  ...
  /**
   * 리스트 엔드포인트 응답을 { [key]: [...], pagination: {...} } 로 정규화.
   * v1 평탄화 응답: data: [...] + 최상위 pagination
   * 구버전 호환:    data: { [key]: [...], pagination: {...} }
   */
  toListResult(key) {
    const out = { [key]: this.getList() };
...
class BaseClient {
   ...
   * @param {Object<string, File|Blob|Buffer>} [options.bindingFiles] — binding 이름 → 파일 매핑 (권장).
   *     서버는 **템플릿이 정의한 binding 이름**(예: 'coverPhoto', 'mainPhoto')을 multipart part name 으로 요구.
   * @param {Array<File|Blob|Buffer>} [options.files] — DEPRECATED. 모든 파일을 단일 fileFieldName 으로 전송 (서버 거부).
   * @param {string} [options.fileFieldName='files'] — DEPRECATED. options.files 와 함께 보존.
   */
  _buildTemplateFormData(templateUid, parameters, options = {}) {
    const fd = new FormData();
    fd.append('templateUid', templateUid);
    fd.append('parameters', JSON.stringify(parameters));

    if (options.bindingFiles) {
      for (const [bindingName, file] of Object.entries(options.bindingFiles)) {
        if (file == null) continue;
        fd.append(bindingName, file);
      }
    }

    if (options.files && options.files.length > 0) {
      // 호환 모드: deprecated 단일 필드명 방식
      const fieldName = options.fileFieldName || 'files';
      // eslint-disable-next-line no-console
      if (typeof process !== 'undefined' && process.emitWarning) {
        process.emitWarning(
          `_buildTemplateFormData files=[...] 는 deprecated 입니다. ` +
          `multipart 파일 part 이름은 템플릿 binding 이름과 일치해야 하므로 ` +
          `bindingFiles={${fieldName === 'files' ? 'coverPhoto' : 'mainPhoto'}: ...} 형태로 전달하세요.`,
          'DeprecationWarning'
        );
      }
